...
| GP3 (pin5) | Armスイッチ | 物理トグルSW → GND（内部プルアップ, ON=Low=Arm） |
...
### Arm操作（物理トグルスイッチ）
- **GP3の物理トグルスイッチ ON=Arm / OFF=Disarm**（CH7）。スイッチ位置＝Arm状態が目視で分かる。
- ゲームパッドAボタンによるArmトグルは**廃止**（押しボタンは誤タッチ1回で即Disarm=墜落のため）。Aボタンは現在未割当。
- 切断インヒビット: ゲームパッド切断中はCH7強制Disarm。スイッチON据え置きのまま再接続しても、**一度OFFに戻すまで再Armしない**（デバッグUARTに `# ARM inhibited...` 表示）。
